...
function buscarLivro() {
    ...
    const resultado = livros.filter(livro => livro.titulo.toLowerCase().includes(termo) || livro.autor.toLowerCase().includes(termo) || livro.genero.toLowerCase().includes (termo) || livro.ano.toString().includes(termo))
         ...
         if (resultado.length > 0) {
            ...
            alert('livros(s) não encontrado(s)!')
            listarLivros(livros)
         }
        }  
